    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp.result === "ok") {
                ...
                $(".notificaciones").html(resp.mensaje);
                $(".notificaciones").show("slow");
                setTimeout(function () {
                    $(".notificaciones").hide("slow");
                    $(".notificaciones").removeClass("alert-success");
                }, 3000);
            } else {
                $(".notificaciones").addClass("alert-warning");
                $(".notificaciones").html(resp.mensaje);
                $(".notificaciones").show("slow");
                setTimeout(function () {
                    $(".notificaciones").hide("slow");
                    $(".notificaciones").removeClass("alert-warning");
                }, 3000);
            }
        },
        error: function () {
            $(".notificaciones").addClass("alert-danger");
            $(".notificaciones").html("Error de sistema. Contactesé por otra vía, disculpe las molestias");
            $(".notificaciones").show("slow");
            setTimeout(function () {
                $(".notificaciones").hide("slow");
                $(".notificaciones").removeClass("alert-danger");
            }, 3000);
        },
        dataType: "json"
    });
